...
router.get('/:productId', async (req, res) => {
  ...
  try {
    const productResult = await pool.query(`
      ...
      GROUP BY p.product_id, c.category_name, s.store_name, p.store_id
...
router.get('/store/:storeId', async (req, res) => {
  ...
  try {
    ...
    const productResult = await pool.query(query, [storeId]);
    ...
    const mediaResult = await pool.query(`
      ...
      WHERE product_id = ANY($1::uuid[])
      ORDER BY product_id, sort_order ASC
    `, [productResult.rows.map(p => p.product_id)]);
    ...
    const fullProducts = productResult.rows.map(p => ({
      ...p,
      media: mediaMap[p.product_id] || []
    }));
